...
{
    int qtdNulos = 0, qtdVotosP = 0, qtdVotosG = 0, i, j, maiorG = 0, maiorP = 0, empateG = 0, empateP = 0;
    ...
    qtdNulos = eleicao->totalEleitores - (qtdVotosP + qtdVotosG);

    maiorP = ObtemVotoPresidente(&eleicao->presidentes[0]);
    presida = *eleicao->presidentes[0];

    for (i = 1; i < eleicao->presidentes; i++)
    {
        if (ObtemVotoPresidente(&eleicao->presidentes[i]) > maiorP)
        {
            maiorP = ObtemVotoPresidente(eleicao->presidentes[i]);
    ...
    maiorG = ObtemVotoGovernador(&eleicao->governadores[0]);
    gov = *eleicao->governadores[0];

    for (i = 1; i < eleicao->governadores; i++)
    {
        if (ObtemVotoGovernador(&eleicao->governadores[i]) > maiorG)
        {
            maiorG = ObtemVotoGovernador(&eleicao->governadores[i]);
            gov = *eleicao->governadores[i];
        }
    }
